function [p,s,v,F] = wjn_plot_surface(filename,color,dec,alpha)
...
if ~isnumeric(color) || numel(color)==3
    p=patch('vertices',s.vertices,'faces',s.faces,'Facecolor',color,'EdgeColor','none','FaceAlpha',alpha);
    v=[];
elseif size(color,2)==4 || length(color)==size(s.vertices,1)
    ...
    ExtrapolationMethod = 'linear'; % only turn on if you know why
     ...
     p=patch('vertices',s.vertices,'faces',s.faces,'CData',v,'FaceColor','interp','EdgeColor','none','FaceAlpha',alpha);
...
elseif size(color,2) == 3 && size(color,1) == size(s.vertices,1)
    v = color;
    p=patch('vertices',s.vertices,'faces',s.faces,'FaceVertexCData',v,'EdgeColor','none','FaceAlpha',alpha);
     
else
    p=patch('vertices',s.vertices,'faces',s.faces,'FaceColor',[.5 .5 .5],'EdgeColor','none','FaceAlpha',alpha);
    v=[];
