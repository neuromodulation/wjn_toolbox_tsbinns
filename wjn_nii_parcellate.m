...
slashes = strfind(parcellation_image, '\');
dots = strfind(parcellation_image, '.');
parcellation_name = parcellation_image(slashes(end)+1:dots(end)-1);
...
for a = 1:length(input_images)
    ...
    fnames{a} = [input_images{a}(1:end-4) '_' parcellation_name '.csv'];
    writetable(T,[input_images{a}(1:end-4) '_' parcellation_name '.csv']);
end
